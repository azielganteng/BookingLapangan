...
<div class="space-y-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-zinc-900">Kelola Lapangan</h1>
            <p class="text-zinc-500 text-xs sm:text-sm mt-1">Daftar semua lapangan yang terdaftar dalam sistem</p>
        </div>
    ...
    {{-- Search & Filter --}}
    <form method="GET" action="{{ route('admin.semua-lapangan') }}" class="flex flex-col sm:flex-row gap-2.5 sm:gap-3">
            ...
            <table class="w-full text-xs sm:text-sm text-left">
                <thead class="bg-zinc-50 text-zinc-500 uppercase text-[11px] font-semibold border-b border-zinc-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 whitespace-nowrap">Jam Operasional</th>
                        <th class="px-4 py-3 whitespace-nowrap">Harga Sewa</th>
                ...
                <tbody class="divide-y divide-zinc-100 text-zinc-700">
                    ...
                    <tr class="hover:bg-zinc-50/80 transition-colors">
                            ...
                            <div class="flex items-center gap-3">
                                @if($item->gambar_url)
                                    <img src="{{ $item->gambar_url }}"
                                         ...
                                         class="w-12 h-12 rounded-lg object-cover shrink-0 border border-zinc-100">
                                @else
                                    <div class="w-12 h-12 bg-zinc-100 rounded-lg flex items-center justify-center text-zinc-400 shrink-0 border border-zinc-100">
                                ...
                                @endif
                                <div class="min-w-0">
                                    <p class="font-bold text-zinc-900 text-xs sm:text-sm truncate">{{ $item->nama_lapangan }}</p>
                                    <p class="text-[11px] text-zinc-400 mt-0.5 line-clamp-1 max-w-xs">{{ $item->deskripsi_lapangan ?? 'Tidak ada deskripsi' }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3.5 text-xs font-medium text-zinc-600 whitespace-nowrap">
                            {{ \Illuminate\Support\Str::substr($item->jam_buka, 0, 5) }} – {{ \Illuminate\Support\Str::substr($item->jam_tutup, 0, 5) }}
                        </td>

                        <td class="px-4 py-3.5 font-bold text-green-600 whitespace-nowrap text-xs sm:text-sm">
                            Rp {{ number_format($item->harga_sewa, 0, ',', '.') }}<span class="text-[11px] text-zinc-400 font-normal">/jam</span>
                        </td>
