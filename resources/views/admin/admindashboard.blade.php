...
<div class="space-y-6 max-w-7xl mx-auto">
    ...
    {{-- Header --}}
    <div>
        <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-zinc-900">Dashboard Admin</h1>
        <p class="text-zinc-500 mt-1 text-xs sm:text-sm">Ringkasan statistik booking dan performa pendapatan lapangan</p>
    </div>

    {{-- Stat Cards --}}
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">

        <div class="bg-white rounded-2xl shadow-xs p-4 sm:p-5 border border-zinc-200 flex flex-col justify-between">
            <p class="text-zinc-400 text-xs font-semibold uppercase tracking-wider">Total Lapangan</p>
            <h2 class="text-2xl sm:text-3xl font-black text-zinc-900 mt-2">{{ $totalLapangan ?? 0 }}</h2>
        </div>

        <div class="bg-white rounded-2xl shadow-xs p-4 sm:p-5 border border-zinc-200 flex flex-col justify-between">
            <p class="text-zinc-400 text-xs font-semibold uppercase tracking-wider">Total Booking</p>
            <h2 class="text-2xl sm:text-3xl font-black text-blue-600 mt-2">{{ $totalBookingSemua ?? 0 }}</h2>
        </div>

        <div class="bg-white rounded-2xl shadow-xs p-4 sm:p-5 border border-zinc-200 flex flex-col justify-between">
            <p class="text-zinc-400 text-xs font-semibold uppercase tracking-wider">Booking Pending</p>
            <h2 class="text-2xl sm:text-3xl font-black text-yellow-600 mt-2">{{ $bookingPending ?? 0 }}</h2>
        </div>

        <div class="bg-white rounded-2xl shadow-xs p-4 sm:p-5 border border-zinc-200 flex flex-col justify-between">
            <p class="text-zinc-400 text-xs font-semibold uppercase tracking-wider">Total Pendapatan</p>
            <h2 class="text-lg sm:text-2xl font-black text-green-600 mt-2 truncate">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-xs p-4 sm:p-5 border border-zinc-200 overflow-hidden">
            <h2 class="text-sm sm:text-base font-bold text-zinc-800 mb-4">Grafik Total Booking</h2>
        ...
        <div class="bg-white rounded-2xl shadow-xs p-4 sm:p-5 border border-zinc-200 overflow-hidden">
            <h2 class="text-sm sm:text-base font-bold text-zinc-800 mb-4">Grafik Pendapatan</h2>
            <div class="overflow-x-auto">
    ...
    <div class="bg-white shadow-xs rounded-2xl overflow-hidden border border-zinc-200">
        ...
        <div class="p-4 sm:p-5 border-b border-zinc-100 flex items-center justify-between">
            <h2 class="text-sm sm:text-base font-bold text-zinc-800">Booking Masuk Terbaru</h2>
            ...
            <table class="w-full text-xs sm:text-sm text-left">
                ...
                <tbody class="divide-y divide-zinc-100">
                    ...
                    <tr class="hover:bg-zinc-50/80 transition-colors">
                        ...
                        <td class="px-4 py-3.5 font-semibold text-zinc-900 whitespace-nowrap">
                            {{ $item->user->name ?? 'Pengguna' }}
                        </td>

                        <td class="px-4 py-3.5 whitespace-nowrap text-zinc-800 font-medium">
